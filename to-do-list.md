...
- **260903-01. 선택형 로컬 하이브리드 의미검색 (Zvec)**
  **목표:** 현재 `docudog_search`의 경로·태그·요약 문자열 매칭을 보완하여, 이미 추출한 문서 본문을 자연어 의도로 찾고 근거 위치까지 반환한다. 기본 동작·기본 설치 용량은 유지한다.

  **범위/설계:**
  - `semantic_search.enabled`는 기본 `false`; `zvec`은 선택 의존성으로 둔다. 인덱스는 `%USERPROFILE%/.docudog/semantic_index/`에만 저장하고, 원본 파일·watch 폴더에는 쓰지 않는다.
  - `extract_document_text()`의 결과를 문단/슬라이드/페이지 등 가능한 위치 단위로 청크화한다. 청크에는 `file_id`, `sha256`, 경로, 위치 anchor, 분석 시각, P등급, 태그, 카테고리, lineage/thread 식별자를 메타데이터로 보관한다.
  - SHA가 바뀐 파일만 기존 청크를 삭제 후 재색인하고, 삭제·미지원·추출 실패 파일은 관련 청크도 제거한다. 문서당 청크 수, 청크 문자 수, 총 인덱스 용량 상한을 설정으로 둔다.
  - Zvec의 벡터+FTS+구조화 필터를 결합한다. 임베딩은 별도 provider 인터페이스로 분리하며 기본은 로컬 전용; 원격 임베딩은 명시적 opt-in 및 전송 경고 없이는 절대 사용하지 않는다. 한국어 질의셋으로 경량 다국어 모델을 먼저 평가한다.
  - P1/P2의 본문 청크는 기본 미색인(또는 메타데이터만)으로 하고, P3/P4만 검색 근거를 반환한다. allowlist·P등급 판정은 **검색 전** 적용한다. Zvec/zg의 독립 MCP를 노출하지 않고 DocuDog MCP 게이트웨이만 유지한다.
  - `docudog_semantic_search(query, filters, limit)`는 파일·위치·짧은 정책 통과 발췌·검색 방식·신선도만 반환한다. 검색 결과에서 해당 문서의 thread/lineage/related 문서를 이어 볼 수 있게 한다.

  **검증:** 한국어 대표 질의(정확 문자열, 동의어, 기간·등급 필터, P1/P2 차단)를 고정 fixture로 만들고, 기존 메타 검색 대비 재현율·지연시간·인덱스 크기를 기록한다. `enabled=false`에서 Zvec import/모델 다운로드/인덱스 생성이 전혀 일어나지 않아야 한다.
